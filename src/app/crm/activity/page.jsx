...
export default async function CrmActivityPage() {
  ...
  return (
    <section style={{ background: "#0f1419", border: "1px solid #1e293b", borderRadius: 14, padding: 18 }}>
          ...
          <thead>
            <tr style={{ borderBottom: "1px solid #1e293b" }}>
              {["Datum", "Klient", "Typ", "Beskrivning", "Skapad av"].map(h => (
                <th key={h} style={{ textAlign: "left", color: "#64748b", fontSize: 12, padding: "0 10px 10px 0", textTransform: "uppercase" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {rows.map(row => {
              ...
              return (
                <tr key={row.id} style={{ borderBottom: "1px solid #141c24" }}>
                  <td style={{ padding: "10px 10px 10px 0", color: "#e2e8f0", fontSize: 13 }}>{row.date || "-"}</td>
                  ...
                  <td style={{ padding: "10px 10px 10px 0", color: "#e2e8f0", fontSize: 13 }}>{row.description || "-"}</td>
      ...
      {rows.length === 0 && <p style={{ color: "#64748b", fontSize: 13, marginTop: 12 }}>Inga aktiviteter ännu.</p>}
    </section>
  );
}
